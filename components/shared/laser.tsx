...
export interface LaserOptions {
  ...
  offset?: number;
  /** Laser glow color as RGB in the 0 to 1 range or any CSS color string. */
...
const DEFAULTS: Required<Omit<LaserOptions, "color">> & { color: [number, number, number] } = {
  speed: 0.3,
  offset: 140,
  color: [0.05, 0.35, 1],
  thickness: 6,
...
type LaserColorInput = LaserOptions["color"];

function normalizeColor(color: LaserColorInput): [number, number, number] {
  if (Array.isArray(color)) return color;
  const value = typeof color === "string" ? color.trim() : "";
  if (!value) return [0.05, 0.35, 1];

  const probe = document.createElement("canvas");
  const ctx = probe.getContext("2d");
  if (!ctx) return [0.05, 0.35, 1];
